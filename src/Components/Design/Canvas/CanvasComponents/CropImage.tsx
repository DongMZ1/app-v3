...
const CropImage = ({ cropImageID, cropImageDesignItems, onClose }: TCropImage) => {
    ...
    const cropImage = async () => {
        const blob: any = await getCroppedImg(imgRef.current as any, crop, 'cropped-image.jpeg');
        console.log(blob)
    }
    ...
    return <div className='p-4 crop-image'>
        ...
        >
            <img ref={imgRef} crossOrigin='anonymous' className='height-70vh' alt='croped-img' src={cropImageDesignItems.filter(each => each.id === cropImageID)?.[0]?.value} />
        </ReactCrop>
